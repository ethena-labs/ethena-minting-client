...
if __name__ == "__main__":
    ...
    type_ = "ALGO"
    mint_block_limit = 2_000_000

    while True:
        multisig_usdt_bal = int(usdt_contract.functions.balanceOf(multisig).call() / 10 ** 6)
        if multisig_usdt_bal > mint_block_limit:
            multisig_usdt_bal = mint_block_limit
        if multisig_usdt_bal < 100000:
            time.sleep(10)
            continue

        rfq_url = f"{ethena_url}rfq?pair=USDT/USDe&type_={type_}&side=MINT&size={multisig_usdt_bal}"
